...
async function getMessages(loop) {
    if (loop.currentchannel != null) {
        ...
        loop.req1 = $.getJSON(apiurl + "/notbot-proxy/notbot/fetchmessages?channel=" + loop.currentchannel + authString + "&random=" + Math.random() * 100, function(data) {
...
async function moreMessages(loop) {
    ...
    if (loop.currentchannel != null) {
        loop.req2 = $.getJSON(apiurl + "/notbot-proxy/notbot/fetchmessages?channel=" + loop.currentchannel + "&before=" + loop.oldestMessage + authString + "&random=" + Math.random() * 100, function(data) {
...
async function checkMessages(loop) {
    ...
    if (loop.currentchannel != null) {
        loop.req3 = $.getJSON(apiurl + "/notbot-proxy/notbot/fetchmessagesnew?channel=" + loop.currentchannel + "&after=" + loop.latestMessage + authString + "&random=" + Math.random() * 100, function(data) {
            data.reverse().forEach(message => {
                if (loop.cachedmessages[message.id] != true) {
                    createMessage(message, false, loop)
                }            
...
async function sendMessage(message, loop) {
    if (loop.currentchannel != null) {
        ...
        ({
            type: "POST",
            url: apiurl + '/notbot-proxy/notbot/sendmessage',
            dataType: 'json',
            async: false,
            contentType: 'application/json',
...
async function createMessage(message, top, loop) {
    ...
    icon.src = "/notbot-proxy/loader.gif"
    if (loop.cachedmessages[message.id] == true) {
        return
    }
    if (top) { 
...
async function getUser(id, callback, webID, msgID, channel, noUnknown) {
    if (webID != null) {
        $.ajax({
            url: apiurl + "/notbot-proxy/notbot/fetchwebhook?id=" + msgID + "&channel=" + channel + authString + "&random=" + Math.random() * 100,
            dataType: "json",
        ...
        $.ajax({
            url: apiurl + "/notbot-proxy/notbot/fetchuser?id=" + id + authString + "&random=" + Math.random() * 100,
            dataType: "json",
            success: function(data) {
                if (data.id == 964949389938405426) {
                    var _data = data
                    $.ajax({
                        url: apiurl + "/notbot-proxy/notbot/fetchinternal?id=" + msgID + authString + "&random=" + Math.random() * 100,
                        dataType: "json",
                        success: function(data) {
...
function updateChannels() {
    if (currentserver != null) {
        $.getJSON(apiurl + "/notbot-proxy/notbot/fetchchannels?server=" + currentserver + authString + "&random=" + Math.random() * 100, function(data) {
            if (oldchannels != data) {
                channelsDiv.innerHTML = ""
...
function updateServers() {
    $.getJSON(apiurl + "/notbot-proxy/notbot/fetchservers" + "?random=" + Math.random() * 100 + authString, function(data) {
        data.forEach(server => {
